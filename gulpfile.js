...
export const copyFavicon = () => {
  return gulp.src(path.src.favicon).pipe(gulp.dest(dirs.dest));
};
...
// Сборка стилей
const styles = () => {
  var plugins = [autoprefixer(), cssnano(), minmax()];
  return gulp
...
const scripts = () => {
  return gulp
    .src([path.src.js, `!${path.src.jsVendor}`])
    .pipe(
      plumber(
        notify.onError({
          title: "JS",
...
const svg = () => {
  return gulp
    .src([path.src.svg, `!${path.src.sprite}`])
    .pipe(svgo())
    .pipe(gulp.dest(path.build.images));
};
